 ...
 * Pre-creates N threads that pull tasks from queue
 * More efficient than creating thread per task
 * Key components: task queue, worker threads, condition variable
 * Interview: Essential pattern for server applications
 * Explain: work queue, mutex protection, cv wait/notify
 */
