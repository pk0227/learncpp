/*
 * DISTRIBUTED COMPUTING SYSTEM WITH CLIENT-SERVER-WORKER ARCHITECTURE
 * 
 * Demonstrates a simple distributed task processing system with three roles.
 * 
 * ARCHITECTURE:
 * - Server: Central coordinator, accepts connections from clients and workers
 * - Worker: Processes tasks, connects to server to receive work
 * - Client: Submits tasks to server, receives results
 * 
 * COMMUNICATION FLOW:
 * 1. Server listens for connections
 * 2. Client connects and sends task
 * 3. Server echoes task back (simplified - no real worker integration)
 * 4. Worker connects and waits for tasks
 * 
 * CONCURRENCY MODEL:
 * - Server uses detached threads for each client (fire-and-forget)
 * - Each connection handled independently
 * - Simple echo pattern (not true task distribution)
 * 
 * SOCKET OPERATIONS:
 ...
 * - No task queue or load balancing
 * - No error handling or reconnection logic
 * - Detached threads can leak resources
 * - No authentication or security
 * - Echo pattern instead of real task distribution
 * 
 * INTERVIEW CONCEPTS:
 * - Client-server vs peer-to-peer architecture
 * - Thread-per-connection vs event-driven models
