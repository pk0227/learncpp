/*
 * FUTURES AND TASK CREATION COMPARISON
 * 
 * This program compares synchronous vs asynchronous execution of CPU-intensive tasks.
 * 
 * KEY CONCEPTS:
 * - std::future<T>: Represents a value that will be available in the future
 * - std::async(): Creates an async task and returns a future
 * - std::launch::async: Forces asynchronous execution (new thread/task)
 * 
 * PERFORMANCE:
 * - Synchronous: Tasks execute one after another (4 seconds total)
 * - Asynchronous: Tasks execute concurrently (~2 seconds total)
 * 
 * INTERVIEW TIP:
 * - std::async is higher-level abstraction than std::thread
 * - Automatically manages thread lifecycle and exception propagation
 * - future.get() throws exception if async task threw one
 */
