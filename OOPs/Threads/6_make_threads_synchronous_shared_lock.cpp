...
void Incrementer()
{
    ...
    for(int i{}; i < 10; i++)
    {
        //std::unique_lock<std::shared_mutex> ul{g_shared_mutex}     // ul scope is limited to for-loop block only
        g_count++;
    }
...
int main()
{
    ...
    for(int i{}; i < max_loop; i++)
    {
        ...
        for(int thr_c{}; thr_c < num_thrs; thr_c++)
        {
            ...
            thr_v.push_back(std::thread(Incrementer));
            thr_v.push_back(std::thread(Reader));
        }
